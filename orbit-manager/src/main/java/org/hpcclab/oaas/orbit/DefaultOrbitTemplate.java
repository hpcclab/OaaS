...
public class DefaultOrbitTemplate implements OrbitTemplate {
  ...
  public OrbitStructure create(DeploymentUnit deploymentUnit) {
    return new DefaultOrbitStructure(this)
  ...
  public static class DefaultOrbitStructure implements OrbitStructure {
    long id;
    String prefix;
    String namespace;
    DefaultOrbitTemplate template;
    ...
    final String ORBIT_LABEL_KEY = "orbit-id";
    final String ORBIT_COMPONENT_LABEL_KEY = "orbit-part";

    public DefaultOrbitStructure(DefaultOrbitTemplate template) {
      this.template = template;
    }

    DefaultOrbitStructure genId() {
      var tsid = template.tsidFactory.create();
      id = tsid.toLong();
      prefix = "orbit-" + tsid;
      return this;
    }

    public DefaultOrbitStructure load(ProtoOrbit orbit) {
      attachedCls.addAll(orbit.getAttachedClsList());
      attachedFn.addAll(orbit.getAttachedFnList());
      return this;
    }

    public DefaultOrbitStructure attach(DeploymentUnit unit) {
    ...
    public void deployShared() throws Throwable {
      ...
      var sec = new SecretBuilder()
        ...
        .withData(map)
        .build();
      template.k8sClient.secrets().resource(sec).create();
    ...
    public void deployObjectModule() throws Throwable {
      ...
      template.k8sClient.resourceList(deployment, invokerSvc, invokerSvcPing)
        .create();
    ...
    public void deployDataModule() throws Throwable {
      ...
      var svc = createSvc(
        ...
        prefix + "-storage-adapter",
        labels);
      template.k8sClient.resourceList(deployment, svc)
        .create();
      k8sResources.add(deployment);
    ...
    private Deployment createDeployment(String filePath,
      ...
      var deployment = template.k8sClient.getKubernetesSerialization()
    ...
    private Service createSvc(String filePath,
                              ...
                              Map<String, String> labels) {
      var is = getClass().getResourceAsStream(filePath);
      var service = template.k8sClient.getKubernetesSerialization()
