...
* `args` is the map (dictionary) of `args` for this invocation steps.



To make this macro function return the object, you can set the `output` to the name of temporary output of any step.

To construct the response body, you can create the list of `DataMapping`.

==== Defining DataMapping Blocks
[source, yaml, lines]
----
- fromObj: <step name>
  transforms:
    - inject: 'newStore' # key to inject
      path: '$.store' # json path
    - inject: 'secondAuthor'
      path: '$.book[1].author'
...
- fromBody: <step name>
  mapAll: true
----
`DataMapping` is used for combine data from one or more sources. The source can be either object data or the response body of any invocation step.

To define the `DataMapping`, each item has to contain either `fromObj` or `fromBody`. The DataMapping also allow you to transform the data using `transforms` field. Otherwise, setting `mapAll = true` will make this block simply copy the entire JSON data.

To define the `transforms` field, you create the list of mapping between injection key and JSON path (see https://github.com/json-path/JsonPath[reference] for JSON path). The original JSON data will be extracted by each step of transform step and inject to new key.

For example, if the input JSON object is:
[source, json]
----
{
  "store": "store name",
...
----
The output of the first `DataMapping` step will be:
[source, json]
----
{
  "newStore": "store name",
