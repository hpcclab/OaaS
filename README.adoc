...
image:https://jitpack.io/v/hpcclab/OaaS.svg[]
...
Overall, the default configuration should be able to use as it is. If you have the deference cluster setup you may need to change the following configurations:
...
NOTE: We make these configuration files by assuming the cluster relies on Rook to provide object storage. In case that you are not using Rook, you have to customize the environment variable section in `sa.yml` to match your object storage.
...
. Expose the OaaS API. This step is depend on your cluster setup. You can expose the API using Ingress by the following example configuration.
+
[source,yaml]
----
