...
You can still use the same command to work with the `image` object
...
== Record Object
The record class is the class without any file declaration, which means it can only hold the JSON data.
...
# CREATE RECORD OBJECT WITH JSON DATA AND SAVE THE NEW OBJECT ID
# MAKE SURE THE COMMAND ESCAPE THE QUOTE CORRECTLY.
ocli o c example.record -d '{"hello": "world"}' -s
