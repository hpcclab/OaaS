...
public class V2TaskEventProcessor {
  ...
  private List<V2TaskEvent> handleCreate(V2TaskEvent taskEvent) {
    ...
    var eventList = taskEventFactory
    ...
    if (version!=null) {
      ...
      success = taskStateRepo.getRemoteCache()
        .putIfAbsent(taskEvent.getId(), taskState)==null;
    }

    LOGGER.debug("processing CREATE event id={} success={} submitting={}", taskEvent.getId(),
      success, submitting);
  ...
  private List<V2TaskEvent> handleNotify(V2TaskEvent taskEvent) {
    var taskStateMetadata = taskStateRepo.getRemoteCache()
      .getWithMetadata(taskEvent.getId());
    var version = taskStateMetadata==null ? null:taskStateMetadata.getVersion();
    var taskState = taskStateMetadata==null ? new TaskState():taskStateMetadata.getValue();
    ...
    if (version!=null) {
      success = taskStateRepo.getRemoteCache()
        .replaceWithVersion(taskEvent.getId(), taskState, version);
    } else {
      success = taskStateRepo.getRemoteCache()
        .putIfAbsent(taskEvent.getId(), taskState)==null;
    }

    LOGGER.debug("processing NOTIFY event id={} success={} submitting={}", taskEvent.getId(),
      success, submitting);
    if (success && submitting) {
      submitTask(taskEvent.getId());
    }
